...
8. 如果字段做了类型转换，就不会走索引。比如 `select a from table where a = 1`，当 a 虽然存储的是数字，但是类型为 varchar 字段时，如果要跟数字 `1` 做对比，就需要将 `a` 字段的内容转换为数字。此时，索引本来是 varchar 的，但是字段转换为了数字，索引就会失效。
9. 使用了 `select *`
