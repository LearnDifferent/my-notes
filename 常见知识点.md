...
* 建表的时候，添加一行：`index 索引名称 列名(长度[可选])`
...
> 相同点：都是实现动态 SQL 的方式，都可以把参数传入 xml 文件中
...
在编程语言中，尤其是在处理对象和数据结构时，常常会遇到拷贝的概念。拷贝可以分为三种类型：引用拷贝（Reference Copy）、浅拷贝（Shallow Copy）、和深拷贝（Deep Copy）。这些拷贝类型在处理对象及其内部引用时有着不同的行为。

引用拷贝 (Reference Copy)：

引用拷贝是最基本的拷贝类型，它仅仅复制对象的引用，而不复制对象本身。在引用拷贝之后，两个引用指向同一个对象，因此对一个引用所做的任何修改都会影响到另一个引用所指向的同一个对象。

```java
Object obj1 = new Object();
Object obj2 = obj1; // 引用拷贝
```

在这个例子中，`obj1` 和 `obj2` 都指向同一个 `Object` 实例。对 `obj1` 或 `obj2` 所做的任何修改都会反映在这个共享实例上。

浅拷贝 (Shallow Copy)：

浅拷贝创建一个新的对象，然后将原始对象的非静态字段复制到新对象。如果字段是基本类型，浅拷贝会复制字段的值；如果字段是引用类型，浅拷贝则复制引用但不复制引用所指向的对象。

```java
ArrayList<Integer> original = new ArrayList<>(Arrays.asList(1, 2, 3));
ArrayList<Integer> shallowCopy = (ArrayList<Integer>) original.clone(); // 浅拷贝
```

在这个例子中，`shallowCopy` 是 `original` 的一个浅拷贝。两个列表共享相同的元素实例。

深拷贝 (Deep Copy)：

深拷贝不仅复制对象本身，还会递归地复制对象内部的所有引用对象。深拷贝之后，原始对象和拷贝对象完全独立，对拷贝对象的修改不会影响原始对象，反之亦然。

实现深拷贝通常需要显式地定义如何复制每个引用对象。这可能包括调用引用对象的拷贝构造函数、拷贝方法或者使用序列化。
...
比较:：
- **引用拷贝**：最简单，仅复制引用，不复制对象。两个引用指向同一个对象。
- **浅拷贝**：创建一个新对象，复制原始对象的字段值。对于引用字段，复制引用但不复制引用指向的对象。
- **深拷贝**：创建一个完全独立的对象副本，包括所有嵌套的对象。最复杂，但可以避免由共享引用引起的副作用。

选择哪种类型的拷贝取决于具体的需求，特别是考虑到对象的独立性和拷贝操作的性能开销。
...
分布式系统（Distributed Systems）和微服务架构（Microservices Architecture）是两个在现代软件开发和系统设计中常被讨论的概念。尽管这两个概念在某些方面是相关联的，但它们指向的是不同的设计理念和目标。

**分布式系统**：

分布式系统指的是一组独立的、相互协作的计算机系统，它们通过网络进行通信和协作，共同完成任务或者提供服务。这些计算机可能分布在世界的不同地方，对于用户来说，看起来就像是一个单一的协作系统。分布式系统的主要目标是通过物理上的分布来实现系统的可伸缩性、可靠性和高效性。

特点包括：

- 分布性：系统的不同部分分布在不同的物理位置。
- 并发性：系统中的多个部分可以同时执行。
- 容错性：系统能够处理部分故障，而不影响整个系统的可用性。
- 透明性：尽管系统分布在不同的节点上，但对用户来说应该表现得像一个统一的系统。

**微服务架构**

微服务架构是一种设计软件的方法，它提倡将单一应用程序分解为一组小的、互相独立的服务，每个服务运行在自己的进程中，通常围绕业务功能构建，通过轻量级的通信机制（通常是HTTP RESTful API）相互协作。每个服务都有自己的数据库和数据管理模型，可以独立于其他服务进行开发、部署和扩展。

特点包括：

- 高度解耦：每个服务都是独立的，对其他服务的依赖最小。
- 独立部署：每个服务可以独立于其他服务部署和更新。
- 技术多样性：每个服务可以使用最适合其需求的技术栈开发。
- 可伸缩性：服务可以根据需求独立扩展。

**分布式系统与微服务的区别**

虽然微服务架构可以被视为分布式系统的一种形式，因为它涉及到多个服务的分布式部署和协作，但主要区别在于它们的关注点和设计目标：

- **关注点**：分布式系统侧重于如何通过网络和计算机的分布来提高系统的整体性能、可靠性和伸缩性。而微服务架构更侧重于应用程序的内部结构，即如何通过将应用分解为一系列小的、独立的服务来提高软件的可维护性、可伸缩性和快速迭代能力。
- **设计目标**：分布式系统的设计目标通常是为了解决大规模数据处理、计算能力分布等问题。而微服务架构的设计目标则是为了提高开发效率、简化部署和支持技术多样性。

总结来说，分布式系统和微服务虽然有交集，但分布式系统更侧重于物理资源和计算能力的分布问题，而微服务架构则侧重于软件开发过程中的服务划分、组织和管理。

## 幂等性

为什么要保证幂等性？

- 因为分布式系统中，总会出现超时的情况，一旦超时就要重试
- 重试的时候，如果一个多次执行同一个请求，就可能造成多次操作导致需要的结果和实际结果不同
...
- 状态机：通过数据状态的流转来驱动业务的流程（比如数据中添加 status 字段来存储状态，每次状态流转就 `update` status 字段，如果发现已经 `update` 了 status，说明新的请求）
- 每次请求都携带 token（唯一标记），通过 token 来判断请求是否重复
- 数据库的唯一约束（唯一索引）
- Redis 的 `setnx`
...
### 连接量大，并发量大的场景，使用配置中心作为缓存

在读多写少，比如每家银行要配置最大金额限制的场景。此时，每家银行的配置，只会每天改 1 到 3 次，也就是“写少”。

因为每次请求，都会去查这个配置，如果连接很多，而且并发流量很高，达到亿级，那么 Redis 这种缓存中间件就会扛不住。

我们可以使用配置中心（Apollo、Nacos 等）来作为缓存，只需要维护配置文件，然后多台机器在连接配置中心的时候，配置中心会通过心跳机制将配置文件的内容推送给机器，这样就实现了超高并发的缓存。

使用配置中心作为缓存，是因为配置中心的性能好，配置修改后还有推送机制，且配置文件可以在本地方便地进行修改。

### 银行的离线全量备份，及在“不可用期间”实现交易

银行一般会在一个时间段进行“定时离线备份”，一般是使用磁带机进行的 **大规模全量备份** ，或者锁全表确保事务完整性来备份，所以整个备份期间业务都不可用，这是为了实现绝对的可靠性和信息安全。

如果要在此期间实现交易的话……

首先 跨行业务，如转账之类的，其实都要走人行认证 ，但工作时间之后人行接口关闭。所以支付宝之类其实都是走 **备付金账户** ，各银行 APP 转账秒也是如此，一般使用 **收单账户付款** 。这种情况下结算和对账相当可怕，就有了一个名为闸金的概念。

> 问：请问这个人行接口关闭是什么意思？转账用的 BEPS 和 IBPS 不是 24×7 吗？
>
> 答：结算是日切走人行，前后不一致。很多地方的每日维护其实是为了每日对账，所以这里表述有问题。

所以，跨行业务能在全天使用是结算打款渠道的不同。支付的话早期没有网联的时候如果单独银行通道维护，还可能需要走 EPOS，代扣等不同的渠道去进行组合扣款，就有了银行通道路由这个有意思的玩意。 银行自身也不是完全全天可用。一般会有定时的备份，以增量为主。护网节假日重大活动等时期禁止上线。

银行自身的维护一般提前 24 小时发公告，主要面向商户，B 端用户。主支付链路基本不动，即使变更也是双写对账，变更一段时期再切换，很多代码设计之初优先考虑回滚等。 所以你能感受到比较好的 SLA。


[参考 X 的这条 Thread](https://twitter.com/amehochan/status/1772799471317090459)

### 金融理财系统的支付流程

支付的请求进来后，由理财服务处理请求。理财服务会先调用账户服务，完成账户资金的冻结。

理财服务在调用了账户服务冻结资金后，会接着调用交易系统。

交易系统会对订单进行拆分（拆单），将交易订单拆分为多个子订单。不同的子订单走不同的交易平台。

在交易系统完成拆单后，交易系统会继续调用支付网关去完成支付。支付网关会将支付的结果，通过 MQ 发送给理财服务。

理财服务在调用交易系统后，会监听支付相关的消息队列，当收到支付成功/失败的消息后，再去调用会计系统等来做后续的处理。
